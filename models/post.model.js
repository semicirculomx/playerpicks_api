...
const postSchema = mongoose.Schema({
    ...
    "is_pick": { type: Boolean, default: false },
    "pick": {
        type: mongoose.Schema.Types.ObjectId,
        ref: 'Pick'
    },
...
postSchema.statics.addOne = async function ({
    ...
    // post.user = user_id;
    // post.id = id;
    // post.id_str = id.toString();
    // return post.save();
    return await mongoose.model('Post').create({
        ...post,
        user: user_id,
        id: id,
        id_str: id.toString()
...
postSchema.statics.searchCategory = async function (category, page = 1) {
    page = parseInt(page);
    return this.find({ pick: category })
        .sort('-created_at')
        .skip(sPage * (page - 1))
        .limit(sPage)
}

postSchema.statics.searchHtml = async function (query, page = 1) {
    page = parseInt(page);
    return this.find({ htmlContent: { $regex: query, $options: "i" } })
        .sort('-created_at')
        .skip(sPage * (page - 1))
        .limit(sPage)
}

postSchema.statics.searchMatch = async function (matchId, page = 1) {
    page = parseInt(page);
    return this.find({ matches: matchId })
        .sort('-created_at')
        .skip(sPage * (page - 1))
        .limit(sPage)
}

postSchema.statics.searchTitle = async function (query, page = 1) {
    page = parseInt(page);
    return this.find({ post_title: { $regex: query, $options: "i" } })
        .sort('-created_at')
        .skip(sPage * (page - 1))
        .limit(sPage)
}
...
postSchema.post('deleteOne', { document: true, query: true }, async doc => {
    try {
        //update statuses_count in User
        // await mongoose.model('User').findOneAndUpdate({ _id: doc.user }, {
        //     $inc: { statuses_count: 1 }
